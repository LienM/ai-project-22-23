...
class EmbeddingCalculator(BatchProcess):
    ...
    def __init__(self, args):
        ...
        self.article_df = self.article_df.set_index(self.article_df['article_id'])
    ...
    def batch_function(self, min_row_ind, batch_nr):
        ...
        img_df = pandas.DataFrame(part_of_df['image_name']).set_index(part_of_df.index)
        ...
        # ensure all article_ids are
        assert sorted(embeddings_df['article_id'].values.tolist()) == sorted(part_of_df['article_id'].values.tolist())
...
if __name__ == '__main__':
    # e.g. python embeddings.py --model VGG16 --nr_rows_per_batch 2000
